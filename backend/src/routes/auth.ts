...
const AuthResponse = Type.Object({
  ...
  accessTokenExpiresIn: Type.Integer({
    description: "Durata in secondi dell'access token.",
  }),
});
  ...
  app.post(
    "/logout-all",
    {
      ...
      schema: {
        ...
        security: [{ bearerAuth: [] }],
        response: {
          200: Type.Object({ ok: Type.Boolean(), revoked: Type.Integer() }),
        },
      },
    },
    async (request) => {
    ...
    "/verify-email",
    {
      schema: {
        ...
        response: {
          ...
          400: Type.String({ contentMediaType: "text/html" }),
        },
      },
    },
    async (request, reply) => {
  ...
  app.get(
    "/reset-password",
    {
      schema: {
        tags: ["auth"],
        summary:
          "Pagina per scegliere una nuova password (aperta dal link ricevuto)",
        querystring: Type.Object({
          token: Type.String({ minLength: 10, maxLength: 200 }),
        }),
